...
$email_test = 'medico@test.it';
$password_test = '123456';

// 1. Test chiamata cURL
echo "<p>1. Interrogazione tabella medici per l'email: <strong>$email_test</strong>...</p>";
$risultato = supabase_request('medici?email=eq.' . urlencode($email_test) . '&select=*');
...
if (isset($risultato['error'])) {
    ...
    if (empty($risultato)) {
        echo "<p style='color:orange;'><strong>Attenzione:</strong> La tabella 'medici' ha risposto ma restituisce un array vuoto. Significa che l'utente con email <code>$email_test</code> non esiste nel database o la RLS sta bloccando la lettura.</p>";
    } else {
        $medico = $risultato[0];
